...
export async function updateMyProfile(formData: FormData) {
    try {
        ...
        // Handle arrays for specialties and removeSpecialties
        const specialties: string[] = [];
        const removeSpecialties: string[] = [];
        
        formData.forEach((value, key) => {
            if (key !== 'file' && value !== '') {
                if (key === 'specialties') {
                    specialties.push(value as string);
                } else if (key === 'removeSpecialties') {
                    removeSpecialties.push(value as string);
                } else {
                    // Handle numeric fields according to IDoctorUpdate schema
                    if (key === 'experience' || key === 'appointmentFee') {
        ...
        // Add arrays to data only if they have values (matching IDoctorUpdate schema)
        if (specialties.length > 0) {
            data.specialties = specialties;
        }
        if (removeSpecialties.length > 0) {
            data.removeSpecialties = removeSpecialties;
        }
