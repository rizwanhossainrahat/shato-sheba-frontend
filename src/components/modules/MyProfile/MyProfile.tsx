...
const MyProfile = ({ userInfo }: MyProfileProps) => {
  ...
  // Specialty management state for doctors
  const [availableSpecialties, setAvailableSpecialties] = useState<ISpecialty[]>([]);
  ...
  // Load specialties and initialize selected ones for doctors
  useEffect(() => {
  ...
  const initializeDoctorSpecialties = () => {
    if (userInfo.doctor?.doctorSpecialties) {
      const specialtyIds = userInfo.doctor.doctorSpecialties.map(
  ...
  const getSpecialtyTitle = (id: string) => {
    ...
    return specialty?.title || "Unknown Specialty";
  };

  const getAvailableSpecialtiesForSelection = () => {
  ...
  const handleRemoveSpecialty = (specialtyId: string) => {
    ...
    // If this was an original specialty, add to removed list
    const originalSpecialtyIds = userInfo.doctor?.doctorSpecialties?.map(ds => ds.specialitiesId) || [];
    if (originalSpecialtyIds.includes(specialtyId)) {
  ...
  const getNewSpecialties = () => {
    const originalSpecialtyIds = userInfo.doctor?.doctorSpecialties?.map(ds => ds.specialitiesId) || [];
    return selectedSpecialtyIds.filter(id => !originalSpecialtyIds.includes(id));
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Add specialty data for doctors
    if (userInfo.role === "DOCTOR") {
      // Only add specialty data if there are changes
      ...
      // Only add removed specialties if there are any
      if (removedSpecialtyIds.length > 0) {
        removedSpecialtyIds.forEach(id => {
          formData.append('removeSpecialties', id);
        });
      }
    }
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        // Only reset specialty state for doctors
        if (userInfo.role === "DOCTOR") {
