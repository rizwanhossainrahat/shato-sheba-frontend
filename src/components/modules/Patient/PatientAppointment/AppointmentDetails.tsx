...
const AppointmentDetails = ({ appointment }: AppointmentDetailProps) => {
  ...
  // Guard clause for schedule dates
  if (!appointment.schedule?.startDateTime || !appointment.schedule?.endDateTime) {
    return (
      <Card>
        <CardContent className="p-6">
          <p className="text-center text-muted-foreground">Invalid appointment schedule data</p>
        </CardContent>
      </Card>
    );
  }
